...
if __name__ == "__main__":
    ...
    pilgrimmenu.add_command(label = "Jaganath Mandir", command = menu.jagnath )
    pilgrimmenu.add_command(label = "sun temple", command = menu.sun)
    pilgrimmenu.add_command(label = "shri Hanuman Mandir", command = menu.hanuman )
    pilgrimmenu.add_command(label = "Dewari Mandir", command = menu.dewri)
    pilgrimmenu.add_separator()
    menubar.add_cascade(label="Relegious", menu=pilgrimmenu)

    parkmenu = tk.Menu(menubar, tearoff = 0, background = 'lightblue', foreground = 'black')
    ...
    fallmenu = tk.Menu(menubar, tearoff = 0, background = 'lightblue', foreground = 'black')
    fallmenu.add_separator()
    fallmenu.add_command(label = "Dasham Fall")
    fallmenu.add_command(label = "Hundru Fall")
    fallmenu.add_separator()
    menubar.add_cascade(label = "Falls", menu = fallmenu)

    hangmenu = tk.Menu(menubar, tearoff = 0, background = 'lightblue', foreground = 'black')
    hangmenu.add_separator()
    hangmenu.add_command(label = "Patratu Valley")
    hangmenu.add_separator()
    menubar.add_cascade(label = "Hangout", menu = hangmenu)

    dammenu = tk.Menu(menubar, tearoff = 0, background = 'lightblue', foreground = 'black')
    dammenu.add_separator()
    dammenu.add_command(label = "Kanke Dam")
    dammenu.add_separator()
    menubar.add_cascade(label = "Dam's", menu = dammenu)

    musmenu = tk.Menu(menubar, tearoff = 0, background = 'lightblue', foreground = 'black')
    musmenu.add_separator()
    musmenu.add_command(label = "State Museam")
    musmenu.add_separator()
